...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Simcord")
	var updateFlag = flag.Bool("repair", false, "Repair Simcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Simcord")
	...
	if *versionFlag {
		fmt.Println("Simcord Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2023 Vendicated and Simcord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Simcord Installer' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Simcord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Simcord will likely not work.")
}
