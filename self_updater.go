...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/Simcord/Installer/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "SimcordInstallerCli.exe", "SimcordInstaller.exe")
		return BaseUrl + filename
	case "darwin":
		return BaseUrl + "SimcordInstaller.MacOS.zip"
	case "linux":
		return BaseUrl + "SimcordInstallerCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "SimcordInstallerUpdate")
	if err != nil {
